 ...
 *
 * <p>Handle URLs with protocol "rsrc". "rsrc:path/file.ext" identifies the content accessible as
...
public class RsrcURLStreamHandler extends java.net.URLStreamHandler {
  ...
  protected void parseURL(URL url, String spec, int start, int limit) {
    String file;
    if (spec.startsWith(JIJConstants.INTERNAL_URL_PROTOCOL_WITH_COLON)) file = spec.substring(5);
    else if (url.getFile().equals(JIJConstants.CURRENT_DIR)) file = spec;
    else if (url.getFile().endsWith(JIJConstants.PATH_SEPARATOR)) file = url.getFile() + spec;
    else file = spec;
    setURL(
        url,
        JIJConstants.INTERNAL_URL_PROTOCOL,
        "",
        -1,
        null,
        null,
