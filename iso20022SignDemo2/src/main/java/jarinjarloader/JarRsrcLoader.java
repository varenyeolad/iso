...
public class JarRsrcLoader {
    ...
    public boolean accept(String library, String actualOsFilter) {
      ...
      for (String libFilter : allFilters) {
        if (library.contains(libFilter)) {
          // debug("found libFilter=" + libFilter + " but valid for [" +
          // libsForFilter.get(actualOsFilter)
          // + "]");
  ...
  public static void main(String[] args)
    ...
    for (int i = 0; i < all.size(); i++) {
      ...
      if (rsrcPath.endsWith(JIJConstants.PATH_SEPARATOR))
        ...
        rsrcUrls[i] =
            new URL(
                JIJConstants.JAR_INTERNAL_URL_PROTOCOL_WITH_COLON
                    + rsrcPath
  ...
  private static ManifestInfo readManifestInfo() throws IOException {
    ...
    while (resEnum.hasMoreElements()) {
      try {
        ...
        if (is != null) {
          ...
          result.rsrcMainClass =
              mainAttribs.getValue(JIJConstants.REDIRECTED_MAIN_CLASS_MANIFEST_NAME);
          String rsrcCP = mainAttribs.getValue(JIJConstants.REDIRECTED_CLASS_PATH_MANIFEST_NAME);
          if (rsrcCP == null) rsrcCP = JIJConstants.DEFAULT_REDIRECTED_CLASSPATH;
          result.rsrcClassPath = splitSpaces(rsrcCP, ' ');
          result.configureConditionalPath(mainAttribs);
          if ((result.rsrcMainClass != null)
              && !result.rsrcMainClass.trim().isEmpty()) // $NON-NLS-1$
          return result;
        }
      } catch (Exception e) {
    ...
    System.err.println(
        "Missing attributes for JarRsrcLoader in Manifest ("
            + JIJConstants.REDIRECTED_MAIN_CLASS_MANIFEST_NAME // $NON-NLS-1$
            + ", "
  ...
  private static List<String> filter(ManifestInfo mi, List<String> all) {
    ...
    debug(
        "accepting only the libraries not matched at all or the ones declared with ["
            + ManifestInfo.CLASS_PATH_FILTER_MATCH_ON
            + libFilter
            + "]");
    ...
    for (String library : all) {
      ...
      if (accepted) result.add(library);
  ...
  /**
   * JDK 1.3.1 does not support String.split(), so we have to do it manually. Skip all spaces (tabs
   * are not handled)
   *
  ...
  private static String[] splitSpaces(String line, char separator) {
    ...
    while (firstPos < line.length()) {
      ...
      if (lastPos == -1) lastPos = line.length();
      if (lastPos > firstPos) {
